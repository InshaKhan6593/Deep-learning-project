...
model = keras.models.load_model(model_path)  # ✅ Correct way to load Keras model
...
def predict_with_gru(input_data, encoder, model):
    ...
    Handles encoding and reshaping automatically
    """
    # Step 1: Encode
    X_encoded = encoder.transform(input_data)
    
    # Step 2: Convert to numpy if DataFrame
    if isinstance(X_encoded, pd.DataFrame):
        X_encoded = X_encoded.values
    
    # Step 3: Reshape for GRU (batch, timesteps, features)
...
if date and time:
    ...
    if map_type == "Complete NYC Map":
        ...
        # ✅ Use custom prediction function for GRU
